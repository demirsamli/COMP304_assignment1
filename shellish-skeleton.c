...
void io_redirection(struct command_t *command){ // 'command' is a pointer to a command_t struct.
  // Declaring 'open' so the compiler allows the call.
  extern int open(const char *pathname, int flags, ...);
  // Defining flag values we pass to 'open', so the calls do the right thing.
  int shell_o_rdonly = 0, shell_o_wronly = 1, shell_o_creat = 64, shell_o_trunc = 512, shell_o_append = 1024;
  int file_descriptor; // small non-negative integer the OS uses to refer to an open file.
  // Here it is used to store the result of 'open'.

  // Now, we have to look for each case (<, >, >> which correspond to redirects[0], redirects[1], and redirects[2] respectively).
  // If the user has given an input redirection, the corresponding redirects[i] value should behold the filename instead of being NULL.
  // We'll make use of that to apply I/O redirection.

  // Case 1: The user has included '<'.
  if ((*command).redirects[0] != NULL){
    // We open the file whose name is in redirects[0] in read-only mode (according to the instructions).
    file_descriptor = open((*command).redirects[0], shell_o_rdonly);
    // If the openning file process doesn't go as expected, we directly exit.
    if (file_descriptor == -1){
      exit(1);
    }
    // If we successfully opened the file, we perform the actual input redirection using 'dup2'.
    dup2(file_descriptor, STDIN_FILENO); // make file_descriptor 0 point to the same open file as file_descriptor
    // “the file descriptor numbered 0” = 'stdin'
    close(file_descriptor);
  }

  // The other 2 cases are almost the same with the first case.
  // Case 2: The user has included '>'.
  if ((*command).redirects[1] != NULL){
    file_descriptor = open((*command).redirects[1], shell_o_wronly | shell_o_creat | shell_o_trunc, 0644);
    if (file_descriptor == -1){
      exit(1);
    }
    // This time we use 'STDOUT' instead of 'STDIN'.
    dup2(file_descriptor, STDOUT_FILENO);
    close(file_descriptor);
  }

  // Case 3: The user has included '>>'.
  if ((*command).redirects[2] != NULL){
    file_descriptor = open((*command).redirects[2], shell_o_wronly | shell_o_creat | shell_o_append, 0644);
    if (file_descriptor == -1){
      exit(1);
    }
    dup2(file_descriptor, STDOUT_FILENO);
